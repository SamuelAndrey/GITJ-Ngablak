...
$sql = query("SELECT * FROM ngm_kel");

if(isset($_POST["tambah"])) {
    if (tambah_ngm_kel($_POST) > 0){
        echo "<script>
                alert('Berhasil menambah data!');
                document.location.href = 'ngm_kel.php';
            </script>";
    } else {
        echo "<script>
        ...
        <table id="tabel_ibd" class="display nowrap" style="width:100%">
          ...
          <?php foreach($sql as $row) : ?>
              <tr>
                  ...
                  <td><?= $row["nama_kel"]?></td>
                  <td>
                    <a href="" type="button" class="btn btn-sm btn-primary"  data-bs-toggle="modal" data-bs-target="#ubahdata" onclick="dataId('<?= $row['kd_kel'];?>')">Ubah</a>
                    <a href="?id=<?= $row["kd_kel"]?>" type="button" class="btn btn-sm btn-secondary" onclick="return confirm('Konfirmasi penghapusan data');">Hapus</a>
                  </td>
              </tr>
          <?php endforeach; ?>
          </tbody>
        ...
        <div class="modal-body">
          <form action="" method="post">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="kd_kel" placeholder="Kode keluarga" name="kd_kel">
            </div>

            <div class="mb-5">
              <label for="nama_kel" class="form-label">Nama Keluarga</label>
              <input type="text" class="form-control" id="nama_kel" placeholder="Nama Keluarga" name="nama_kel">
            </div>
